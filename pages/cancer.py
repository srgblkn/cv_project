...
# -----------------------------
# Styling: background + opaque UI + high contrast
# -----------------------------
def apply_background_and_contrast(bg_path: Optional[Path]) -> None:
    bg_css = ""
...
# -----------------------------
# Compact YAML parsing for key params (без pyyaml)
# -----------------------------
def parse_yaml_shallow(path: Path) -> Dict[str, str]:
    ...
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if not v or v.endswith("{") or v.endswith("["):
            continue
        out[k] = v
    return out
...
def extract_predictions(result) -> Tuple[List[Tuple[int, int, int, int]], List[str], Optional[pd.DataFrame]]:
    """
    Универсально пытаемся извлечь:
    - boxes (xyxy)
    - labels для боксов (class + conf)
    - классификационную таблицу top-k (если она доступна)

    Возвращаем:
    - boxes_xyxy (list)
    - box_labels (list)
    - cls_df (DataFrame или None)
    """
...
# Header
# -----------------------------
opaque_card("Medical Scan Analyzer", "Пакетная обработка изображений: локализация подозрительных зон и оценка класса результата.")
...
# -----------------------------
# Training params + charts (по запросу)
# -----------------------------
...
if run_btn:
    ...
    for idx, up in enumerate(uploads, start=1):
        try:
            ...
            # собираем CSV-строку (универсально)
            top_class = None
            top_prob = None
